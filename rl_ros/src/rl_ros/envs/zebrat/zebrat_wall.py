...
from openai_ros.robot_envs import zebrat_env
...
class ZebratWallEnv(zebrat_env.ZebratEnv):
    def __init__(self):
        """
        ...
        ros_ws_abspath = rospy.get_param("/zebrat/ros_ws_abspath", None)
        ...
        LoadYamlFileParamsTest(rospackage_name="openai_ros",
                               rel_path_from_package_to_file="src/openai_ros/task_envs/zebrat/config",
                               yaml_file_name="zebrat_wall.yaml")

        # Here we will add any init functions prior to starting the MyRobotEnv
        super(ZebratWallEnv, self).__init__(ros_ws_abspath)

        # Only variable needed to be set here
        number_actions = rospy.get_param('/zebrat/n_actions')
        ...
        #self.offset = [0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15]
    ...
    def _set_action(self, action):
        ...
        elif action == 3: #RACKWARD
            linear_speed = -self.linear_turn_speed
            angular_speed = 0.0
            self.last_action = "BACKWARD"
        elif action == 4: #LEFTBACK
            linear_speed = -self.linear_turn_speed
            angular_speed = self.angular_speed
            self.last_action = "LEFTBACK"

        elif action == 5: #RIGHTBACK
            linear_speed = -self.linear_turn_speed
            angular_speed = -1*self.angular_speed
            self.last_action = "RIGHTBACK"
    ...
    def _compute_reward(self, observations, action, done):
        ...
        if not done:
            ...
            if crash != True:
                ...
                if (action == 0) or (action == 1) or (action == 2):
                    reward2 += 0.6 *(decay2 *observations[16] + math.pow(decay2,2) * (observations[17] + observations[15])+ math.pow(decay2,3)* (observations[18]+ observations[14])+ math.pow(decay2,4)* (observations[19]+ observations[13])+ math.pow(decay2,5)* (observations[20]+ observations[12])   )                
                ...
                '''   
                    ...
                    reward += +(0.8*observations[5] + 0.5 * (observations[4] + observations[6])+ 0.1* (observations[3]+ observations[7]))   
        ...
        # NOt DONE
            ...
            if observations[20] + observations[12]  >= 7:
