...
class Agent:
    def __init__(self, alpha=0.0003, beta=0.0003, input_dims = 2,
                 ...
                 batch_size=64, reward_scale=2, chkpt_dir='/home/tian/simulation_ws/src/zebrat/zebrat_training/scripts/SAC/models/'):
        self.gamma = gamma
        self.tau = tau
    ...
    def load_models(self):
        ...
        rospy.logerr("model loaded, self.actor.max_action: " + str(self.actor.max_action))
    ...
    def store_transition(self, state, action, reward, new_state, done):
        rospy.logerr('IN storing')
        rospy.logerr('old state: ' + str(state))
        rospy.logerr('Action: ' + str(action))
        rospy.logerr('Reward: ' + str(reward))
        rospy.logerr('new state' + str(new_state))
        rospy.logerr('Done: '+str(done))
